    ...
    var controller = {
        ...
        template: {
            ...
            mainInfoHtml: function (data) {
                ...
                var tempHtml = '';
                tempHtml = '<section class="tc-baseInfo"><span class="preliminary-advice">初诊建议<i></i></span>' +
                    ...
                    if (rep.responseObject.responseStatus) {
                        ...
                        if (data.patientCasemap) {
                           container.append(that.template.mainInfoHtml(data.patientCasemap));
                            //添加初诊建议按钮
                            $(".preliminary-advice").on("click",function () {
                                window.location.href = "https://m.allinmed.cn/pages/myServices/check_suggestion.html?caseId=" + common.getpara().caseId+"&type=native";
                            });
                        }
                        //添加症状描述
                        ...
                        if (data.patientCasemap) {
                            ...
                            //app端查看大图用
                            if($(".app-checkDatum .uploadListsBox")&&$(".app-checkDatum .uploadListsBox li").length>0){
                              $(".app-checkDatum .uploadListsBox li").each(function (index,element) {
                                var imgSrc1= $(element).children("img").attr("src");
                                that.config.imgArray1.push(imgSrc1);
                              })
                            }
                            if($(".app-patientPhoto .uploadListsBox")&&$(".app-patientPhoto .uploadListsBox li").length>0){
                              $(".app-patientPhoto .uploadListsBox li").each(function (index,element) {
                                var imgSrc2= $(element).children("img").attr("src");
                                that.config.imgArray2.push(imgSrc2);
                              })
                            }
                        }
                        //添加基本信息
                        ...
                        //添加继续咨询
                        container.append("<section class='appGoRefer'>继续沟通</section>");
                        that.appNative();
                    }
                }
            })
            ...
            setupWebViewJavascriptBridge(function (bridge) {
                ...
                bridge.registerHandler('returnCaseId', function (data, responseCallback) {
                    ...
                    if (!common.getpara().caseId){
                        window.location.href='https://m.allinmed.cn/pages/directOperation/reservation_list.html?caseId='+JSON.parse(data).caseId+'&isOrder='+JSON.parse(data).isOrder;
                    }
                    var responseData = {'resourceUrl': 'https://m.allinmed.cn/pages/directOperation/reservation_list.html?caseId='+JSON.parse(data).caseId+'&isOrder='+JSON.parse(data).isOrder};
                    // log('JS responding with', responseData)
                    responseCallback(responseData)
                });
                bridge.callHandler('changeNavTitleAction',{'title': "问诊单"},function (response) {

                });
                $(".preliminary-advice").on("click",function () {
                    bridge.callHandler('changeNavTitleAction',{'title': '初诊建议'},function (response) {

                    })
                });
                $(".app-checkDatum li").on("click",function () {
                    bridge.callHandler('gotoImageBrowser',{'index': $(this).index(),"urlArray":that.config.imgArray1},function (response) {

                    });
                });
                $(".app-patientPhoto li").on("click",function () {
                    bridge.callHandler('gotoImageBrowser',{'index': $(this).index(),"urlArray":that.config.imgArray2},function (response) {

                    });
                });
                $(".appGoRefer").on("click",function () {
                    bridge.callHandler('backAction',function (response) {
