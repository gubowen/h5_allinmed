...
(function ($) {
    var Validate = function (element, config) {
        var that = this;
        this.config = $.extend(false, $.fn.validate.config, config);
        ...
        addValidate: function () {
            ...
            $(this.config.rules).each(function (index, element) {
                ...
                if (msg) {
                    ...
                    that.removeValidateTips(msg);
                }
            })
        },
        removeValidateTips: function (tips) {
    ...
    var VerifiPolicy = {
        ...
        isMobile: function (value, errorMsg) {
            if (!(/^(127|13[0-9]|14[0-9]|15[0-9]|17[0-9]|18[0-9])\d{8}$/).test(value)) {
                return errorMsg;
            }
        },
        //判断输入的没有数字
        noNumber: function (value, errorMsg) {
            if ((/^(?=.*\d.*\b)/).test(value)) {
                return errorMsg;
            }
        },
        //特殊符号
        special: function (value, errorMsg) {
            if ((/[`~!！？@#$%^&*()_+<>?:"{},，。.\/;'[\] ]/).test(value)) {
                return errorMsg;
            }
        },
        isEmoji: function (value, errorMsg) {
            if ((/\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDE4F]/g).test(value)) {
                return errorMsg;
            }
        },
        ...
        //判断固定长度
        valueLength: function (value, errorMsg, length) {
            if (value.length != length) {
                return errorMsg;
            }
        },
        ...
        //判断输入前后是否有空格
        nameSpace: function (value, errorMsg) {
            if (/(^\s+)|(\s+$)/g.test(value)) {
                return errorMsg;
            }
        },
