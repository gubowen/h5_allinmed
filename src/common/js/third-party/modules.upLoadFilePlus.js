 ...
 */
import $ from 'jquery';
import toJSON from 'jquery-json';
/**
 * @Desc：
 * @Usage:
 * @Notify：
    ...
    var container = {
        ...
        fileUpload: function (ev) {
            ...
            $('.ev-upLoadAdd').on("click",function(){
                var _this = $(this),
                    _inputYes = $(this).siblings('.ev-upLoadInput'),
                ...
                if(_inputYes.length>0){
                    ...
                    $(this).siblings('.ev-upLoadInput').trigger("click");
                  $(this).siblings('.ev-upLoadInput').on("change",function () {
                    var files =_this.siblings('.ev-upLoadInput').files[0].files[0];
                    _t.localReview();
                    _t.upLoadPic();
                    console.log(files);
                  })
        ...
        fileDel: function () {
            var _t = this;
            $('.tc-upLoadBox').on("click", ".qu-upLoadDel",function () {
        ...
        deleteImg:function(_thisElem){
            var _t = this,
                _index = _thisElem.closest('.qu-upLoadItemList').index(),
                _treatmentId;   //当前图片index
            //保存文件信息处理
            if(!_thisElem.closest('.qu-upLoadItemList').hasClass('tc-historyImg')){
                var _historyListLength = _thisElem.closest('.qu-upLoadItemList').find('.qu-upLoadItemList.qu-historyImg').length;
                if(_historyListLength>0){
                ...
                if(Obj.upLoadType == "3" || Obj.upLoadType == "5"){
                    ...
                    _treatmentId = _thisElem.closest('.qu-upLoadItemBox').siblings('.qu-upLoadTitle').children('.tc-upLoadTitleName').data("id");
                }
                switch (_treatmentId){
                    case 0:
                        _t.op.files0.splice(_index,1);
                        break;
                    case 1:
