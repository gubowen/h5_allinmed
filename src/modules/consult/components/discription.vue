    ...
    methods: {
        ...
        api.ajax({
          ...
          beforeSend(config) {
            ...
            if (localStorage.getItem("selectList")) {
              ...
              that.secondQuestionList = JSON.parse(localStorage.getItem("secondList"));
              that.questionList = JSON.parse(localStorage.getItem("questionList"));
      ...
      otherReason(pIndex, index, e) {
        ...
        if (api.getByteLen(content) > 1000) {
          this.questionList[pIndex].optionList[index].optionDesc = api.getStrByteLen(content, 1000);
          this.errorShow = true;
          setTimeout(() => {
      ...
      getPainLevelData(item, value) {
        ...
        this.secondQuestionList[item.optionId].optionIdList.push(this.painProgress[value].optionId);
        this.secondQuestionList[item.optionId].optionDesc = this.painProgress[value].optionName + this.painProgress[value].optionDesc;
      },
      //数据装载...
      paramsInstall() {
        ...
        localStorage.setItem("secondList", JSON.stringify(this.secondQuestionList));
      ...
      answerValidator() {
        ...
        for (let index = 0; index < this.selectList.length; index++) {
          if (this.selectList[index].optionIdList.length === 0 /*|| this.selectList[index].optionDesc.length === 0*/) {
            ...
            if (this.$el.querySelectorAll(".consult-total")[index].querySelector(".second-wrapper") && !this.$el.querySelectorAll(".consult-total")[index].querySelector(".second-wrapper .selected")) {
              this.errorShow = true;
              setTimeout(() => {
                this.errorShow = false;
              }, 2000);
              ...
              flag = false;
              break;
            } else if (this.$el.querySelectorAll(".consult-total")[index].querySelector(".consult-question-item.selected textarea") && this.$el.querySelectorAll(".consult-total")[index].querySelector(".consult-question-item.selected textarea").value.length === 0) {
              this.errorShow = true;
              setTimeout(() => {
                this.errorShow = false;
              }, 2000);
