...
//                store.commit("stopLastTimeCount");
          ...
          beforeSend(config) {
            ...
            if (data.responseObject && data.responseObject.responseData) {
              ...
              if (dataList && dataList.length !== 0) {
                ...
                if (that.userData.nick.length === 0) {
                  that.getPatientBase();
                }
              }
            }
      ...
      sendPayFinish(count){
        ...
        if (parseInt(count.orderType) === 0) {
          contentDesc = '患者已申请您的免费问诊';
        } else {
          contentDesc = `患者已购买您的${desc}问诊（${count.orderAmount}元）`;
        }
        this.nim.sendCustomMsg({
          scene: 'p2p',
          to: that.targetData.account,
          ...
          content: JSON.stringify({
            ...
            data: {
              ...
              subContentDesc: `患者已购买了您的${desc}问诊`,
            }
          }),
          done (error, msg) {
            if (!error) {
