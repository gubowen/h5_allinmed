<template>
  <section class="doctorContent">
    <section class="doctorContent-box">
      <section class="doctor-personalInfo">
        <div class="doctor-personalInfoBox">
          <div class="doc-personalInfo-left">
            ...
            <div class="doc-presentInfo">
              <span class="doc-major" v-show="platformId.length>0">{{(platformId == 1 ? "骨科" : "手外科")}}</span><span
              class="doc-presents"
              v-show="medicalTitleNewArr[0]&&medicalTitleNewArr[0].length>0">{{medicalTitleNewArr[0]}}</span><span
              class="doc-presents-two"
              v-show="medicalTitleNewArr[1]&&medicalTitleNewArr[1].length>0">{{medicalTitleNewArr[1]}}</span><span
              class="doc-presents-two"
              v-show="medicalTitleNewArr[2]&&medicalTitleNewArr[2].length>0">{{medicalTitleNewArr[2]}}</span>
            </div>
          </div>
        ...
        <div class="doc-personal-labelBox" v-show="hospitalLevel.length>0||isTop==1">
          <span class="doc-perLabel-item" v-show="hospitalLevel.length>0">{{hospitalLevel}}</span><span
          class="doc-perLabel-item" v-show="isTop==1">全国top10骨科医院</span>
        </div>
      </section>
      <!--网上问诊-->
      <section class="online-visitsBox">
        <section class="doc-commonTitle">
          ...
          <span class="doc-lastPerson" v-show="isShowCureStatus"
                :class="{'isUsable-num':isUsableNum,'not-usable-num':isNotUsable,'not-usable-off':isOpenCure}">{{isCureStatusText}}</span>
        </section>
        <section class="doc-onlineContentBox">
          ...
          <section class="doc-onlineForChart">
            ...
            <section class="onlineForChart-right">
              <p @click="payPopupShow=!isNotUsable&&!isOpenCure,payType='pay'"
                 :class="{'notUsableCure':isNotUsable||isOpenCure}">去问诊</p>
            </section>
          </section>
          <!--免费问诊-->
          <section class="doc-onlineForFree" v-show="inquiryState>0">
            <section class="onlineForFree-ticket"
                     @click="payPopupShow=!isUseCureForFree&&!isNotUsable&&!isOpenCure,payType='free'"
                     :class="{'isUse':isUseCureForFree,notUsable:isNotUsable||isOpenCure}"></section>
          </section>
        </section>
      </section>
      <!--专科信息-->
      <section class="college-infoBox doc-commonSty"
               v-show="areasExpertise.length>0||illnessMapList.length>0||operationMapList.length>0||precedingYearOperationNum.length>0||yesteryearOperationNum.length>0">
        ...
        <section class="doc-collegeBox">
          <section class="doc-collegeBoxItem" v-show="illnessMapList.length>0">
            ...
            <p class="collegeItem-right"><span
              v-for="(item,index) in illnessMapList">{{item.illnessName}}{{((index == illnessMapList.length - 1) ? "" : "、")}}</span>
            </p>
          </section>
          <section class="doc-collegeBoxItem" v-show="operationMapList.length>0">
            <p class="collegeItem-left">擅治手术</p>
            <p class="collegeItem-right"><span
              v-for="(item,index) in operationMapList">{{item.operationName}}{{((index == operationMapList.length - 1) ? "" : "、")}}</span>
            </p>
          </section>
          <p class="doc-collegeBoxItem-totalNum" v-show="precedingYearOperationNum>0||yesteryearOperationNum>0">
            近年独立完成的骨科手术病历数</p>
          <section class="doc-medicalNumTotalBox" v-show="precedingYearOperationNum>0||yesteryearOperationNum>0">
            <section class="doc-medicalNumBox">
              ...
              <span class="doc-delTextBox"><span class="doc-medicalNumTotal">{{precedingYearOperationNum}}</span><span
                class="doc-medicalNumText">例</span></span>
            </section>
            <section class="doc-medicalNumBox">
              <span class="doc-medicalYear">{{precedingYear}}年</span>
              <span class="doc-delTextBox"><span class="doc-medicalNumTotal">{{yesteryearOperationNum}}</span><span
                class="doc-medicalNumText">例</span></span>
            </section>
          </section>
        </section>
    ...
    <!--支付弹层-->
    <payPopup v-if="payPopupShow" @paySuccess="paySuccessBack" @docCallBack="docStatusChange"
              :payPopupShow.sync="payPopupShow" :payPopupParams="{
        docName:fullName,
        docId:docId,
        caseId:caseId,
        patientId:patientId,
